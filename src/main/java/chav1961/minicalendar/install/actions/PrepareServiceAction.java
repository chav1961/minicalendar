...
public class PrepareServiceAction implements ActionInterface<InstallationDescriptor>{
	...
	public PrepareServiceAction(final Localizer localizer, final ErrorProcessing<InstallationDescriptor, InstallationError> err) {
		if (localizer == null) {
	...
	public Class<?>[] getAncestors() {
		return new Class<?>[] {CopyApplicationContentAction.class, PrepareDatabaseAction.class} ;
	}
	...
	private boolean executeWindows(LoggerFacade logger, InstallationDescriptor content) throws IOException {
		final File				tempDir = new File(System.getProperty("java.io.tmpdir"));
		final File				configFile = File.createTempFile("winsl", ".conf"); 
		
		fillConfigFile(configFile, content);
		
		final ProcessBuilder	pb = new ProcessBuilder("java", "-jar", "winsl.jar", "install", "-conf", configFile.getName(), "-serviceName", content.serviceName).directory(tempDir).redirectError(Redirect.INHERIT);
	...
	private void fillConfigFile(final File configFile, final InstallationDescriptor content) throws IOException {
		...
		try(final InputStream			is = this.getClass().getResourceAsStream("serviceconfig.txt");
